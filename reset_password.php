...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($errors)) {
    ...
    if (empty($new_password) || strlen($new_password) < 8) {
        ...
        if ($stmt->execute()) {
            ...
            $stmt->execute();
            $stmt->close();
            $success = 'Password reset successfully. You can now <a href="' . SITE_URL . '/login.php" class="text-primary-red hover:underline">log in</a>.';
        } else {
...
<head>
    ...
    <title>Reset Password - DGC Transports</title>
        ...
        <div class="container mx-auto">
            <div class="card p-6 sm:p-8">
                <h1 class="text-3xl font-bold text-gray-800 mb-6 text-center">
                    <i class="fas fa-lock text-primary-red mr-3"></i>Reset Your Password
                </h1>
                <?php if (!empty($errors)): ?>
                    <?php foreach ($errors as $error): ?>
                        <p class="error-message"><i class="fas fa-exclamation-circle mr-1"></i><?= htmlspecialchars($error) ?></p>
                    <?php endforeach; ?>
                    <p class="text-center text-gray-600">Request a new reset link <a href="<?= SITE_URL ?>/forgot_password.php" class="text-primary-red hover:underline">here</a>.</p>
                <?php elseif ($success): ?>
